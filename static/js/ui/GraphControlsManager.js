...
export class GraphControlsManager {
  ...
  initialize() {
    ...
    // Check if initial graph is loaded and update status if so
    const currentGraphId = this.uiManager.graphManager.getCurrentGraphId();
    const currentGraphName = this.uiManager.graphManager.getCurrentGraphName();
    
    if (currentGraphId) {
      this.updateGraphStatus(
        currentGraphName || 'Unnamed Graph',
        this.uiManager.graphManager.hasUnsavedChanges()
      );
      
      // Ensure buttons are in correct state
      if (this.uiManager.elements.saveGraphBtn) {
        this.uiManager.elements.saveGraphBtn.disabled = !this.uiManager.graphManager.hasUnsavedChanges();
      }
    } else {
      this.updateGraphStatus('No Graph Loaded', false);
      if (this.uiManager.elements.saveGraphBtn) {
        this.uiManager.elements.saveGraphBtn.disabled = true;
      }
    }
  }
  ...
  addGraphStatusIndicator() {
    ...
    // Save references to the UIManager elements collection as well
    if (this.uiManager && this.uiManager.elements) {
      this.uiManager.elements.currentGraphName = graphNameDisplay;
      this.uiManager.elements.graphModifiedIndicator = modifiedIndicator;
    }
  }
  ...
  updateGraphStatus(graphName, isModified) {
    console.log(`Updating graph status: name=${graphName}, modified=${isModified}`);
    ...
    if (this.uiManager.elements.saveGraphBtn) {
      // Enable save button if there's a graph loaded and it's modified
      const graphId = this.uiManager.graphManager.getCurrentGraphId();
      this.uiManager.elements.saveGraphBtn.disabled = !graphId || !isModified;
    }
  }
   ...
   * @param {string} status - The status ('Executing', 'Completed', etc.)
  ...
  updateStatus(status, className) {
    ...
    if (statusElement) {
      ...
      statusElement.classList.remove('executing', 'success', 'error');
