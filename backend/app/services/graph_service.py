...
# Fix for the node duplicate ID issue in graph_service.py (or repositories.py)
...
def create_node(graph_id, node_id, name, backend, model, system_message=None, 
    ...
    """Create a new node in a graph, handling potential duplicates gracefully."""
    try:
        # First check if node with this ID already exists
        existing_node = Node.query.get(node_id)
        
        if existing_node:
            # If node exists and belongs to this graph, update it instead
            if existing_node.graph_id == graph_id:
                # Update the existing node
                existing_node.name = name
                existing_node.backend = backend
                existing_node.model = model
                existing_node.system_message = system_message
                existing_node.temperature = temperature
                existing_node.max_tokens = max_tokens
                
                # Only update position if provided
                if position_x is not None:
                    existing_node.position_x = position_x
                if position_y is not None:
                    existing_node.position_y = position_y
                
                db.session.commit()
                return existing_node
            else:
                # Node exists but belongs to a different graph - generate new ID
                import time
                new_node_id = f"node-{int(time.time() * 1000)}"
                current_app.logger.info(f"Node ID collision detected. Generated new ID: {new_node_id}")
            ...
            # Node doesn't exist, create it normally
            node = Node(
                id=node_id,
                graph_id=graph_id,
                name=name,
